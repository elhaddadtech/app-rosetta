...
// To return the data, you can do something like:
  ...
  public function calculateNotes($institution, $file) {
    ...
    $results = DB::table('results as r')
      ...
      ->whereRaw('LOWER(i.libelle) = LOWER(?)', [strtolower($institution)])
      ->whereRaw('LOWER(r.file) = LOWER(?)', [strtolower($file)])
      ->groupBy('u.email', 'l.libelle', 'r.id', 'r.student_id', 'r.language_id')
      ->get();

    // Insert missing records into courses table and calculate note_ceef
    foreach ($results as $result) {
      $exists = DB::table('courses')->where('result_id', $result->result_id)->exists();
      ...
      if (!$exists) {
        ...
        DB::table('courses')->where('result_id', $result->result_id)->update(['note_ceef' => $note_ceef]);
      }
    }

    return response()->json([
      'status'  => true,
      'message' => 'Records vérifiés, insérés et notes calculées avec succès',
    ]);
  }
  ...
  //     ->leftJoin('branches as b', 's.branch_id', '=', 'b.id')
  //     ->leftJoin('semesters as sm', 's.semester_id', '=', 'sm.id')
  //     // Join the courses table to get noteCC1 and noteCC2
  //     ->leftJoin('courses as c', function ($join) {
  //       $join->on('c.result_id', '=', 'r.id');
  //     })
  //     ->select([
  //       'u.email as user_email',
  //       'l.libelle as language_libelle',
  ...
  //     ->whereRaw('LOWER(i.libelle) = LOWER(?)', [strtolower($institution)])
  //     ->whereRaw('LOWER(r.file) = LOWER(?)', [strtolower($file)])
  //     ->groupBy('u.email', 'l.libelle', 'r.student_id', 'r.language_id')
  //     ->get();

  //   // Process results and calculate note_ceef
  //   foreach ($results as &$result) {
  //     // Ensure note_ceef is set before accessing
  //     $result->note_ceef = null;

  //     // Extract and clean noteCC1, noteCC2
  //     $noteCC1      = isset($result->noteCC1) ? (int) $result->noteCC1 : 0;
  //     $noteCC2      = isset($result->noteCC2) ? (int) $result->noteCC2 : 0;
  //     $score_test_4 = isset($result->score_test_4) ? explode('/', $result->score_test_4)[0] : null;
  //     $score_test_4 = is_numeric(trim($score_test_4)) ? (int) trim($score_test_4) : null;
  //     $language     = strtolower($result->language_libelle);
  //     $semester     = isset($result->semester_libelle) ? strtolower($result->semester_libelle) : null;

  //     // If both CC scores are 0, handle the logic for updating note_ceef
  //     if ($noteCC1 == 0 && $noteCC2 == 0) {
  //       // If note_ceef exists but needs to be updated (if it is 'Abs' or another default value)
  //       if ($result->note_ceef === 'Abs' || $result->note_ceef === 'pas encore' || $result->note_ceef === 'Semestre non défini') {
  //         $result->note_ceef = '0'; // Set to 0 if conditions met (update to the appropriate value)
  //       }
  //     }
